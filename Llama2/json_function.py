...
class json_for_database():
    LOCAL_DIRECTORY_DATABASE = f'{os.getcwd()}/Llama2/file_json/database'

    def write_data(self, data, title=LOCAL_DIRECTORY_DATABASE):
        with open(f"{title}.json", "w", encoding="utf-8") as file:
            json.dump(data, file, indent=2, ensure_ascii=False)

    def load_data_all(self, title=LOCAL_DIRECTORY_DATABASE):
        with open(f"{title}.json", "r") as file:
            data = json.load(file)
        return data

    def merge_data(self, data_json, id_user='id_user', title=LOCAL_DIRECTORY_DATABASE):
        # Если файл существует и не пустой
        try:
            with open(f"{title}.json", encoding="utf-8") as file:
                data = json.load(file)
    ...
    def load_data_for_id_user(self, id_user, title=LOCAL_DIRECTORY_DATABASE):
        try:
            with open(f"{title}.json", "r", encoding="utf-8") as file:
                data = json.load(file)
                # temp = data[id_user]
    ...
    def delete_data_for_id_user(self, id_user, title=LOCAL_DIRECTORY_DATABASE):
        try:
            with open(f"{title}.json", encoding="utf-8") as file:
                data = json.load(file)
                new_data = {}
